...
const DATA = [
  ...
  appt('2026-09-13', '11:00', [['עיצוב גבות', 80], ['הסרת שיער', 150]], 'cash', 'pending'),
  appt('2026-09-13', '12:00', [['טיפול פנים', 250]], 'bank', 'cancelled'),      // לא נספר
  appt('2026-09-15', '10:00', [['הסרת שיער', 120]], 'meshulam', 'confirmed'),
  appt('2026-09-19', '10:00', [['עיצוב גבות', 90]], 'cash', 'confirmed'),        // שבת, סוף השבוע
  ...
  appt('2026-08-31', '10:00', [['טיפול פנים', 999]], 'cash', 'confirmed'),       // חודש קודם
  appt('2026-10-01', '10:00', [['טיפול פנים', 777]], 'cash', 'confirmed'),       // חודש הבא
...
test('סיכום יומי: סכום, מספר טיפולים, אמצעי תשלום, ביטולים לא נספרים', () => {
  ...
  assert.deepEqual(s.byPayment, { paybox: 250, cash: 230, bank: 0, meshulam: 0, none: 0 });
});
...
test('סיכום שבועי: סכום, אמצעי תשלום, סוג טיפול, פירוט ימים', () => {
  ...
  assert.equal(s.cancelledCount, 1);
  assert.deepEqual(s.byPayment, { paybox: 250, cash: 320, bank: 0, meshulam: 120, none: 0 });
...
test('סיכום חודשי: סכום, אמצעי תשלום, סוג טיפול, שבועות', () => {
  ...
  assert.deepEqual(s.byPayment, { paybox: 550, cash: 320, bank: 0, meshulam: 120, none: 200 });
...
test('מחירים עם אגורות מסוכמים בלי שגיאות עיגול', () => {
  const d = [appt('2026-05-05', '10:00', [['א', 0.1], ['ב', 0.2]], 'cash', 'confirmed')];
...
test('תבנית הודעה וקישור וואטסאפ', () => {
  const a = appt('2026-09-20', '14:30', [['טיפול פנים', 250], ['עיצוב גבות', 80]], 'cash');
...
test('זיהוי חפיפה בין תורים', () => {
  const day = [
    appt('2026-09-20', '10:00', [['פנים', 250, 60]], 'cash', 'confirmed'),
    appt('2026-09-20', '12:00', [['גבות', 80, 20]], 'cash', 'cancelled'),
  ];
...
test('תזכורות למחר: מי לא קיבלה תזכורת ומי לא אישרה', () => {
  const d = [
    appt('2026-09-20', '11:00', [['א', 1]], 'cash', 'pending', { reminderSent: true }),
    appt('2026-09-20', '09:00', [['א', 1]], 'cash', 'confirmed', { reminderSent: true }),
    appt('2026-09-20', '10:00', [['א', 1]], 'cash', 'pending', { reminderSent: false }),
    appt('2026-09-20', '12:00', [['א', 1]], 'cash', 'cancelled'),
    appt('2026-09-21', '12:00', [['א', 1]], 'cash', 'pending'),
  ];
...
test('אינדקס לקוחות וחיפוש', () => {
  const d = [
    { ...appt('2026-09-01', '10:00', [['א', 100]], 'cash', 'confirmed'), clientName: 'דנה לוי', phone: '050-1111111' },
    { ...appt('2026-09-10', '10:00', [['א', 150]], 'cash', 'cancelled'), clientName: ' דנה  לוי', phone: '' },
    { ...appt('2026-09-05', '10:00', [['א', 200]], 'cash', 'confirmed'), clientName: 'רותם', phone: '052-2222222' },
  ];
